...
class DiscretePointMassModel(BaseDynamics):
    """Point-mass dynamics with discrete-time first-order thrust filtering.

    State layout (9-dim): [p(3), v(3), a_thrust(3)]
    Action layout (3-dim): a_thrust_cmd in m/s^2 (world frame).
    """
    ...
    @property
    def Rz(self) -> Tensor:
        """Rotation matrix about z-axis from quaternion (w,x,y,z -> rotation of thrust direction)."""
        q = self.quat_xyzw
        w, x, y, z = q[..., 3], q[..., 0], q[..., 1], q[..., 2]
        z_vec = torch.zeros_like(w)
        z_vec[..., 2] = 1.0
        yaw = torch.atan2(
            2 * (w * z_vec[..., 0] + x * z_vec[..., 1] - y * z_vec[..., 2]), w**2 + x**2 - y**2 - z**2 + 1e-7
        )
    ...
    def _compute_Rz_local(self) -> Tensor:
        """Compute z-axis rotation matrix for local action frame."""
        q = self.quat_xyzw
        if self.n_agents == 1:
            w, x, y, z = q[3], q[0], q[1], q[2]
        else:
    ...
    def _point_mass_quat(self, a: Tensor, orientation: Tensor) -> Tensor:
        """Compute drone pose quaternion from thrust direction and orientation."""
    ...
    def step(self, U: Tensor) -> None:
        p, v, a_thrust = self._p, self._v, self._a_thrust
        dt = self.dt
        G_vec = self._G_vec
        D = self._D
        lmbda = self._lmbda
        ...
        a_thrust_cmd = U
        next_a = torch.lerp(a_thrust, a_thrust_cmd, control_delay_factor.unsqueeze(-1)) - D * v
