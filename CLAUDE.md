...
DiffAero Lab is an Isaac Lab extension for drone racing simulation. It provides custom multi-agent RL environments built on top of Isaac Lab/Isaac Sim (Nvidia's Omniverse robotics simulation platform).

## Architecture

```
source/diffaero_lab/diffaero_lab/    # Main package (double-nested structure)
├── __init__.py                      # Package export surface
├── tasks/
│   ├── direct/                      # Direct RL tasks
│   │   └── drone_racing/
│   │       ├── drone_racing_env.py      # Environment implementation
│   │       ├── drone_racing_env_cfg.py  # PhysX route config
│   │       ├── drone_racing_env_warp_cfg.py
│   │       ├── agents/                  # RL config entry points
│   │       ├── dynamics_bridge/         # Physics-route bridge layer
│   │       ├── mdp/                     # Rewards, resets, terminations, observations
│   │       └── state/                   # policy / critic / sim_state builders
│   └── manager_based/
├── common/
│   ├── sim_contract.py              # Shared sim-state and capability contract
│   └── rollout_route.py             # Differential route selection
├── uav/
│   ├── assets/                      # Asset catalog and local USD references
│   ├── dynamics/                    # Drone dynamics and controllers
│   └── route_registry.py            # Route-to-bridge registry
└── algo/
    ├── algorithms/                  # RL algorithm implementations
    ├── configs/                     # Algorithm configurations
    ├── trainers/                    # Training loops
    └── wrappers/                    # Env wrappers
```

## OVERVIEW

Isaac Lab extension template for robotics simulation research. Extends Isaac Lab with custom multi-agent RL environments and Omniverse UI extensions. This is the active development workspace; `refer/` contains borrowed reference code.Isaac UV env in source /home/tong/tongworkspace/diffaero_lab/refer/IsaacLab/env_isaaclab/bin/activate。当设计Isaac生态时总是使用context7或deepwiki查询文档进行确认

## ANTI-PATTERNS (THIS PROJECT)

- DO NOT set agent_spec directly (refer/P2M/resources/envs/isaac_env.py:224)
- DO NOT change extension loading order (refer/P2M/resources/envs/isaac_env.py:326)
- Environment must always create one simulation context (direct_rl_env.py, manager_based_env.py)
